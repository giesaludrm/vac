...
def filtro_vacuna(df):
    neumo = df.loc[df["Vacuna"] == "Neumocócica polisacárida 23V"]
    makes = list(df['Vacuna'].drop_duplicates())
    f_vac = st.sidebar.selectbox('Seleccione una Vacuna:', options=makes)
    df_filter = df.loc[df["Vacuna"] == f_vac]

    return neumo if not f_vac else df_filter

def filtro_dosis(df):
    default = df["Dosis"].iloc[0]
    makes = list(df['Dosis'].drop_duplicates())
    f_vac = st.sidebar.selectbox('Seleccione una Dosis:', options=makes)
    df_filter = df.loc[df["Dosis"] == f_vac]

    return default if not f_vac else df_filter
...
df_filtro = df[["Servicio","Comuna","Vacuna"]]
df_filtro2 = df[["Servicio","Comuna"]]
df_filtro3 = df[["Servicio","Comuna", "Vacuna", "Dosis"]]
...
vac_inf_geo_gb = vac_inf_geo.groupby(["Comuna", "Establecimiento","Vacuna", "latitud", "longitud"]).sum("N° de vacunados").reset_index()
...
# TABLA DE DATOS
da_table =  df[["Servicio", "Comuna", "Vacuna","N° de vacunados", "Población objetivo","Avance vacunación"]]
...
#df_criterios = df_criterios.groupby("Vacuna").sum("N° de vacunados").reset_index()
#fig3 = px.pie(df_criterios, values='N° de vacunados', names='Vacuna', title="Vacunas administradas según criterio de elegibilidad")
...
fig_ind = go.Figure(go.Indicator(
    ...
    gauge = {'axis': {'range': [0, 100],'tickvals': [0, 25, 50, 75, 95 ,100]},'threshold' : {'line': {'color': "red", 'width': 2}, 'thickness': 0.75, 'value': 95}},
    #domain = {'x': [0, 1], 'y': [0, 1]},
